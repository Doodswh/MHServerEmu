...
namespace MHServerEmu.Commands.Implementations
{
    ...
    public class StashCommands : CommandGroup
    {
            ...
            foreach (PrototypeId stashRef in stashRefs)
            {
                ...
                foreach (var entry in stash)
                {
                    ...
                    InventoryResult result = Inventory.ChangeEntityInventoryLocation(
                        item, stash, entry.Slot, ref stackEntityId, true);
        ...
        private bool TryMoveItemToStash(Item item, Inventory stash)
        {
            ulong? stackEntityId = null;
            InventoryResult result = Inventory.ChangeEntityInventoryLocation(
                item, stash, Inventory.InvalidSlot, ref stackEntityId, true);
        ...
        private string RepairStash(Player player, string stashName)
        {
            ...
            foreach (PrototypeId stashRef in allStashRefs)
            {
                ...
                if (DecodeUrlString(GenerateStashName(stash, protoName))
                    .Equals(stashName, StringComparison.OrdinalIgnoreCase))
                {
            ...
            using var itemsToMoveHandle = ListPool<ulong>.Instance.Get(out List<ulong> itemsToMove);
            foreach (var entry in targetStash)
            {
                if (entityManager.GetEntity<Item>(entry.Id) != null)
                    itemsToMove.Add(entry.Id);
            }
            ...
            foreach (ulong itemId in itemsToMove)
            {
                ...
                {
                    Logger.Warn($"[STASH-REPAIR] Failed to move item {itemId}: {result}");
                }
            }
        ...
        private string ExtractAvatarNameFromStash(string stashProtoName)
        {
            ...
            int startIndex = stashProtoName.IndexOf(prefix);
            if (startIndex < 0) return string.Empty;

            startIndex += prefix.Length;
            return startIndex < stashProtoName.Length
                ? stashProtoName.Substring(startIndex).Replace("/", "").Trim()
                : string.Empty;
        }

        private string ExtractHeroInternalName(PrototypeId stashProtoRef)
        {
            string protoName = GameDatabase.GetPrototypeName(stashProtoRef);
            const string prefix = "PlayerStashForAvatar";

            int index = protoName.IndexOf(prefix, StringComparison.OrdinalIgnoreCase);
            if (index != -1)
            {
                return protoName.Substring(index + prefix.Length).Replace("/", "").Trim();
            }
            return string.Empty;
        }
